...
public class Conta {
	...
	public void depositar(double valor) {
		if (valor <= 0) {
			System.err.println("Impossível realizar depósito! Valor menor ou igual a zero.");
		} else {
			this.setSaldo(this.getSaldo() + valor);
			atualizarConta(agencia.buscarConta(this.numeroConta)); // Atualiza conta
	...
	public void listarMovimentacoes() {
		if (movimentos.isEmpty() == false) {
			...
			System.err.println("Nenhuma movimentação realizada!");
		}
	}
	...
	public void realizarOperacoes(Conta c, Agencia a) {
		...
		while (ver == true) {
			System.out.print("---------------------------------\nCliente: " + c.pessoa.getNome() + "\nConta Nº: "
					+ c.getNumeroConta() + " / Agência: " + a.getNumero() + " / Saldo atual: R$ " + c.getSaldo()
